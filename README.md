...
| 项目 | 核心定位 | 详情 |
|------|---------|------|
| ECC | Agent Harness 性能优化系统（156+ Skills, 25+ Agents, 持续学习） | [详情](repos/ecc.md) |
| Superpowers | Agent 开发方法论框架（brainstorm→plan→TDD→review, 57k⭐） | [详情](repos/superpowers.md) |
| Skills | Matt Pocock 的可组合 Agent Skills 集合（20k⭐） | [详情](repos/skills.md) |
| Hermes Agent | Nous Research 自进化 Agent（学习循环 + 多平台 + Serverless） | [详情](repos/hermes-agent.md) |
| CodeGraph | 预索引代码知识图谱，减少 Agent 92% 工具调用 | [详情](repos/codegraph.md) |
| Graphify | 多模态知识图谱 Skill（代码+文档+图片+视频, 71x token 减少） | [详情](repos/graphify.md) |
| Claude-Mem | Agent 跨 Session 持久化记忆（72k⭐, 5 阶段 Hook 系统） | [详情](repos/claude-mem.md) |
| OpenClaw | 自托管 AI Agent 框架（350k⭐, Local-first, 50+ 通道） | [详情](repos/openclaw.md) |
